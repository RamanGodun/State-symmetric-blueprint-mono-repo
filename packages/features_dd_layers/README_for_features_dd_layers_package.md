# Features DD Layers Package

**Features DD Layers** is the shared package for **domain** and **data** layers of app features.
...
Add `features_dd_layers` to your app via local path:

```yaml
# apps/<your_app>/pubspec.yaml
dependencies:
  features_dd_layers:
    path: ../../packages/features_dd_layers
```
...
> **Import rule:** Always use barrels from `features_dd_layers_barrels/`.
...
- `lib/features_dd_layers.dart` — main entry point re-exporting all features.
- Each feature has its own barrel exposing only domain & data layer contracts and implementations.

```
features_dd_layers/lib
├─ features_dd_layers.dart                              # 🌐 Root barrel (main public API)
│
├─ features_dd_layers_barrels/                          # 🧰 Public barrels per feature
│  ├─ auth/
...
```dart
import 'package:features_dd_layers/features_dd_layers_barrels/auth/auth.dart';
import 'package:features_dd_layers/features_dd_layers_barrels/auth/auth_infra.dart';
...
```dart
import 'package:features_dd_layers/features_dd_layers_barrels/email_verification/email_verification.dart';
...
```dart
import 'package:features_dd_layers/features_dd_layers_barrels/profile/profile.dart';
...
# Analyze & test
melos exec --scope="features_dd_layers" -- flutter analyze
melos exec --scope="features_dd_layers" -- flutter test
```
