...
```
overlays/
├── core/                                        # Core overlay system components
│   ├── _context_x_for_overlays.dart             # Main context extensions API
│   ├── _overlay_base_methods.dart               # Low-level overlay methods
│   ├── enums_for_overlay_module.dart            # Priority, policies, categories enums
│   ├── global_overlay_handler.dart              # Screen-wide tap handling & keyboard dismiss
│   ├── platform_mapper.dart                     # Platform-specific widget resolver
│   └── tap_through_overlay_barrier.dart         # Tap-through behavior implementation
│
├── overlays_dispatcher/                         # Overlay queue & dispatcher logic
│   ├── overlay_conflicts_strategy.md            # Documentation for conflict resolution
│   ├── overlay_dispatcher.dart                  # Main dispatcher logic & queue management
│   ├── policy_resolver.dart                     # Conflict resolution logic
│   └── overlay_entries/                         # Overlay entry implementations
...
│
├── overlays_presentation/                       # Presentation layer & UI components
│   ├── overlay_presets/                         # Styling presets
│   │   ├── overlay_preset_props.dart            # Props data class for presets
│   │   └── overlay_presets.dart                 # Built-in styling presets (Error, Success, etc.)
...
│       └── simple_animated_banner/              # Animated banner implementation
│           ├── overlay_service.dart             # Service for animated banners
│           └── overlay_widget.dart              # Widget for animated banners
│
├── utils/                                       # Utility functions & helpers
│   ├── lock_controller.dart                     # Lock controller for overlay state
│   ├── overlay_logger.dart                      # Debug logging utility
│   ├── overlay_utils.dart                       # General utility functions
│   ├── overlays_cleaner_within_navigation.dart  # Navigation observer for auto-cleanup
│   ├── show_overlay_after_frame_x_on_context.dart # Extension for post-frame overlay display
│   └── ports/                                   # Port interfaces & locators
│       ├── locker_while_active_overlay.dart     # Locker interface for overlay state
│       ├── overlay_activity_port.dart           # Port for overlay activity tracking
│       └── overlay_dispatcher_locator.dart      # Dispatcher locator interface
│
└── README__for_overlays_module.md               # Module documentation
```
