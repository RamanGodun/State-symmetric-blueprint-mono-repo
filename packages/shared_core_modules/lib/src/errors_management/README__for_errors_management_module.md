...
errors_management/
│
├── core_of_module/                                    # Core error handling infrastructure (immutable)
│   ├── _errors_handling_entry_point.dart              # Entry point: run error handling pipeline
...
│   │
│   └── core_utils/                                    # Utility tools for error handling
│       ├── consumable/                                # One-shot error wrapper for UI feedback
│       │   ├── consumable.dart                        # Consumable implementation
│       │   └── One_time_error_displaying.md           # Guide: one-time UI feedback
│       │
│       ├── errors_observing/                          # Error logging & observability
│       │   ├── loggers/                               # Crash analytics & error logging
│       │   │   ├── errors_log_util.dart               # Error logging utility
│       │   │   └── failure_logger_x.dart              # Failure logger extension
│       │   └── result_loggers/                        # Functional result loggers
│       │       ├── async_result_logger.dart           # Async result logger (for Futures)
│       │       └── result_logger_x.dart               # Extension: result logging (sync)
│       │
│       ├── extensions_on_either/                      # Either extensions and helpers
...
│       │
│       ├── extensions_on_failure/                     # Failure-specific extensions
...
│       └── typedefs_for_errors_management.dart        # Type aliases for error handling
│
├── extensible_part/                                   # Extensible implementations (add project-specific errors here)
│   ├── exceptions_to_failure_mapping/                # Exception → Failure mappers
│   │   ├── _exceptions_to_failures_mapper_x.dart     # Central exception→failure mapping
│   │   ├── dio_exceptions_mapper.dart                # Dio-specific HTTP exceptions mapping
│   │   ├── firebase_exceptions_mapper.dart           # Firebase-specific exceptions mapping
│   │   └── platform_exeptions_failures.dart          # Platform-specific exceptions mapping
│   │
│   ├── failure_extensions/                           # Domain-specific failure extensions
│   │   ├── failure_diagnostics_x.dart                # Diagnostics & metadata for failures
│   │   ├── failure_icons_x.dart                      # Declarative icons for failures (UI)
│   │   └── failure_led_retry_x.dart                  # Retryability logic for failures
│   │
│   └── failure_types/                                # Domain-specific failure types
│       ├── failure_codes.dart                        # Central registry of all error codes
│       ├── firebase_failure_types.dart               # Firebase FailureTypes
│       ├── misc_failure_types.dart                   # Miscellaneous FailureTypes
│       └── network_failure_types.dart                # Network FailureTypes
│
└── README__for_errors_management_module.md           # Main documentation for the module
```
...
TODO: to analyze fpdart package ...
