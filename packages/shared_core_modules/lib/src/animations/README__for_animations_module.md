...
```
├── README__for_animations_module.md
│
├── module_core                                    # Core animation engine contracts and base implementations
│   ├── animation__engine.dart                     # Sealed animation engine contract (API for all engines)
│   ├── animation_base_engine.dart                 # Base engine with shared animation lifecycle logic
│   ├── engine_configs.dart                        # Centralized animation configs (duration, curves) by overlay/platform
│   └── fallback_engine.dart                       # Fallback engine for missing platform-specific implementations
│
├── overlays_animation                             # Overlay-specific animation logic
│   ├── animation_wrapper
│   │   ├── animated_overlay_shell.dart            # Animation shell wrapper for overlays
│   │   ├── animated_overlay_wrapper.dart          # Wrapper managing full animation lifecycle
│   │   └── overlay_animation_x_for_widget.dart    # Extension methods for overlay animation integration
│   ├── engine_mapper_x_on_context.dart            # BuildContext extension for platform-aware engine selection
│   └── overlays_animation_engines                 # Platform-specific animation engines
│       ├── android_animation_engine.dart          # Android-style overlay animations
│       └── ios_animation_engine.dart              # iOS-style overlay animations
│
├── page_transition.dart                           # Page transition animations for navigation
│
└── widget_animations                              # Widget-level animation helpers
    └── widget_animation_x.dart                    # Extension-based DSL for composable widget animations
```
